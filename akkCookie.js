...
// 判断环境变量里面是否有akkck
if (process.env.AKK_COOKIE) {
    if (process.env.AKK_COOKIE.indexOf('&') > -1) {
        CookieAKKS = process.env.AKK_COOKIE.split('&');
    } else if (process.env.AKK_COOKIE.indexOf('\n') > -1) {
        CookieAKKS = process.env.AKK_COOKIE.split('\n');
    } else {
        CookieAKKS = [process.env.AKK_COOKIE];
    }
}
...
CookieAKKS = [...new Set(CookieAKKS.filter(item => !!item))]
console.log(`\n====================共${CookieAKKS.length}个akk账号Cookie=========\n`);
...
for (let i = 0; i < CookieAKKS.length; i++) {
    //
    // if (!CookieAKKS[i].match(/pt_pin=(.+?);/) || !CookieAKKS[i].match(/pt_key=(.+?);/)) console.log(`\n提示:akkcookie 【${CookieAKKS[i]}】填写不规范,可能会影响部分脚本正常使用。正确格式为: pt_key=xxx;pt_pin=xxx;（分号;不可少）\n`);
    const index = (i + 1 === 1) ? '' : (i + 1);
    exports['CookieAKK' + index] = CookieAKKS[i].trim();
}
